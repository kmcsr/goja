...
func (r *Registry) Enable(runtime *goja.Runtime) *RequireModule {
	rrt := &RequireModule{
		r:           r,
		runtime:     runtime,
...
func (r *RequireModule) require(call goja.FunctionCall) goja.Value {
...
func (r *RequireModule) Require(p string) (ret goja.Value, err error) {
...
func Require(runtime *goja.Runtime, name string) goja.Value {
	if r, ok := goja.AssertFunction(runtime.Get("require")); ok {
		mod, err := r(goja.Undefined(), runtime.ToValue(name))
		if err != nil {
			panic(err)
		}
		return mod
